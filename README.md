...
whose gates need Windows, Python with python-chess, and the reference engines.
Run that locally before a pull request.
...
That packages the executables, `machete` and `binpack`, but not the network: a
release needs `net/machete.nnue` attached beside the archive (the engine loads it
from its own folder), until the network is embedded in the binary. `release/`
holds the README and notes that go with it.

## License

MIT, copyright Nick Drohan. See [LICENSE](LICENSE).
