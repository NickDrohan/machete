...
# `bash harness/longqueue.sh EG-01` starts at that stage and skips the ones
# before it, which is how a queue interrupted by something more important -
# a match the owner asked for - picks up again without redoing finished work.
from="${1:-}"
reached=""
stage() {
    if [[ -z "$from" || -n "$reached" || "$1" == "$from" ]]; then
        reached=1
        return 0
    fi
    return 1
...
say "waiting for whatever is running now${from:+ (resuming at $from)}"
wait_quiet
